...
const SOURCE_LABEL = { qr: 'QR scan', link: 'Report link', file: 'Shared file', share: 'Shared file', manual: 'Entered by hand' };
...
function playerReportUrl(playerId, language) {
  return PLAYER_REPORT_BASE + '?player_id=' + encodeURIComponent(playerId) + '&language=' + encodeURIComponent(language || 'en');
}
...
// One-at-a-time picker shared by the metric rail and the time-range rail.
// The arrows, the arrow keys and a horizontal swipe all step through items.
function fillWheel(host, items, selectedId, onPick, config) {
  const settings = config || {};
  const noun = settings.noun || 'option';
  const listLabel = settings.listLabel || 'Options';
  ...
  const previous = el('button', 'wheel__nav', '');
  previous.type = 'button';
  previous.setAttribute('aria-label', 'Previous ' + noun);
  previous.title = 'Previous ' + noun;
  previous.appendChild(wheelArrow(-1));
  previous.addEventListener('click', () => onPick(items[(selectedIndex - 1 + items.length) % items.length].id));
  ...
  const next = el('button', 'wheel__nav', '');
  next.type = 'button';
  next.setAttribute('aria-label', 'Next ' + noun);
  next.title = 'Next ' + noun;
  next.appendChild(wheelArrow(1));
  next.addEventListener('click', () => onPick(items[(selectedIndex + 1) % items.length].id));
...
const Capture = (() => {
  ...
  async function startReading(job) {
    ...
    openReview({
      ...
      source: job.mode === 'file' ? 'file' : job.mode === 'manual' ? 'manual' : job.mode === 'report-url' ? 'link' : 'qr',
      previewUrl,
      note
    });
  }
...
/* ------------------------------------------------------------------ */
/* Radial actions on the scan button                                   */
/* ------------------------------------------------------------------ */
...
const ScanFan = (() => {
  ...
  function init() {
    ...
    /* Mouse: hovering the hub opens the fan, leaving it closes the fan. */
    ...
    /* Touch and mouse: long press opens without also firing the tap action. */
...
async function saveReview() {
  ...
  toast('Reading saved');
  Route.go('dashboard');
  if (Settings.get().mirror && Auth.isConfigured && Auth.isConfigured()) mirrorSync().catch(() => {});
}
...
function bindEvents() {
  ...
  $('#btn-sync-now').addEventListener('click', async () => {
    ...
    if (result.ok) {
      ...
      toast('Synced ' + result.count + ' reading' + (result.count === 1 ? '' : 's') + ' · ' + uploaded + ' up · ' + downloaded + ' down');
    }
